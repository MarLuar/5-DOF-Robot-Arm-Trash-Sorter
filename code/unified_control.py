#!/usr/bin/env python3
"""
5-DOF Robotic Arm - Unified Control System v1.00.03
Clean, stable version with:
...
class UnifiedControlSystem:
    def __init__(self, root):
        self.root = root
        self.root.title("5-DOF Robotic Arm - Unified Control System v1.00.03")
        ...
        # Log
        self.log("System initialized - v1.00.03")
    ...
    def setup_manual_tab(self):
        ...
        # Sequence control
        seq_frame = ttk.LabelFrame(right_frame, text="Sequences", padding="5")
        ...
        ttk.Button(seq_frame, text="Play", command=self.play_sequence).pack(pady=2)
...
3. Go to Auto Detection tab to test
"""
    ...
    def setup_sequences_tab(self):
        ...
        self.cell_listbox = tk.Listbox(self.seq_tab, height=16, width=15)
        self.cell_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        ttk.Label(mid_frame, text="Steps:").pack(anchor='w', pady=(10,0))
        ...
        ttk.Button(mid_frame, text="Save Sequence", command=self.save_current_sequence).pack(pady=5)
        
        # Right: Presets
        right_frame = ttk.LabelFrame(self.seq_tab, text="Quick Presets", padding="10")
        right_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=5, pady=5)
        
        ttk.Button(right_frame, text="Load Rest", command=self.load_rest_preset).pack(fill=tk.X, pady=2)
        ttk.Button(right_frame, text="Load Pickup", command=self.load_pickup_preset).pack(fill=tk.X, pady=2)
        
        ttk.Separator(right_frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)
        
        ttk.Button(right_frame, text="Test Sequence", command=self.test_sequence).pack(fill=tk.X, pady=2)
    ...
    def setup_auto_tab(self):
        ...
        self.auto_check = ttk.Checkbutton(ctrl_frame, text="Auto-Detect",
        ...
        self.stats_label = ttk.Label(stats_frame, text="Objects: 0\nLast cell: N/A")
        self.stats_label.pack()
        
        # Action
        action_frame = ttk.LabelFrame(right_frame, text="Action", padding="5")
        action_frame.pack(fill=tk.X, pady=10)
        
        self.execute_on_detect_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(action_frame, text="Execute on detection",
    ...
    def update_calib_preview(self):
        ...
        try:
            ...
            if not hasattr(self, 'calib_canvas'):
                return
            ...
            scale = min(canvas_width / self.original_width, canvas_height / self.original_height)
    ...
    def on_calib_click(self, event):
        ...
        if hasattr(self, 'calib_x_offset'):
    ...
    def calculate_grid(self):
        ...
        calib_data = {'grid_intersections': {}, 'timestamp': time.time()}
    ...
    def capture_empty_grid(self):
        ...
        if ret:
            ...
            messagebox.showinfo("Success", "Empty grid updated!")
    ...
    def load_sequences(self):
        ...
        if os.path.exists(LEGACY_SEQUENCES_FILE):
            try:
                ...
                for name, steps in legacy_seqs.items():
                    ...
                    for step in steps:
                        if isinstance(step[0], str):
                            ...
                            if angles:
                                self.sequences[name].append({'angles': angles, 'delay': delay})
                        else:
                ...
                self.log(f"Could not load sequences: {e}")
    ...
    def get_preset_angles(self, preset_name):
        ...
        defaults = {'Rest': DEFAULT_REST, 'Pickup': DEFAULT_PICKUP}
    ...
    def add_current_step(self):
        ...
        self.log(f"Added step to {self.current_cell}")
    ...
    def test_sequence(self):
        ...
        self.log(f"Testing: {self.current_cell}")
    
    # Auto Detection Methods
    def toggle_auto_detect(self):
        """Toggle auto detect"""
        self.auto_detect_enabled = self.auto_detect_var.get()
        self.log(f"Auto-detect {'ON' if self.auto_detect_enabled else 'OFF'}")
    ...
    def detect_objects(self, frame):
        ...
        for cnt in contours:
            ...
            if area > self.min_area_var.get():
                ...
                objects.append({'x': x, 'y': y, 'w': w, 'h': h, 'cx': cx, 'cy': cy, 'area': area, 'cell': cell})
    ...
    def detect_object_now(self):
        """Detect now"""
        ret, frame = self.auto_cap.read() if hasattr(self, 'auto_cap') and self.auto_cap else None
        if not ret:
            ret, frame = self.cap.read()
        if not ret:
            return
        ...
        if not objects:
            ...
            for i, obj in enumerate(objects):
                ...
                self.stats_label.config(text=f"Objects: {self.detection_count}\nLast: {self.last_cell}")
                
                if self.execute_on_detect_var.get() and obj.get('cell') in self.sequences:
                    self.log(f"Object in {obj['cell']} - executing!")
                    self.execute_cell_sequence(obj['cell'])
        
        self.log(f"Detection: {len(objects)} objects")
    ...
    def execute_cell_sequence(self, cell):
        ...
        self.log(f"Sequence complete: {cell}")
